    ...
    public void onDraw(Canvas canvas) {
        ...
        if (mVSizes != null && !mVSizes.isEmpty()) {
            ...
            for (int i = 1; i < mVSizes.size(); i++) {
                ...
                canvas.drawLine(
                        (lastVSize.timestamp - minTimestampMs) * xPerMs,
                        min_y - (lastVSize.vsize - vsizeMin) * yMultiplier,
                        (currentVSize.timestamp - minTimestampMs) * xPerMs,
                        min_y - (currentVSize.vsize - vsizeMin) * yMultiplier,
                        mLinePaint);
            }

            // draw marker
            int markerVSize = mVSizes.get(closestToMarker).vsize;
