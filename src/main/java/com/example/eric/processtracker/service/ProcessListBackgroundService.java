...
public class ProcessListBackgroundService extends Service {
    ...
    private ProcessListDB mProcessListDB;
    ...
    public void onCreate() {
        mProcessListDB = new ProcessListDB(getApplicationContext());
    ...
    private void fetchProcessList() {
        ...
        // append values to db
        SQLiteDatabase db = mProcessListDB.getWritableDatabase();
        db.beginTransaction();
        try {
            for (ProcessEntry processEntry : processEntries) {
                ContentValues contentValues = new ContentValues();
                contentValues.put(ProcessListDB.KEY_NAME, processEntry.name);
                contentValues.put(ProcessListDB.KEY_PID, processEntry.pid);
                contentValues.put(ProcessListDB.KEY_VSIZES, processEntry.vsize);
                contentValues.put(ProcessListDB.KEY_TIME, timestampMs);
                db.insert(ProcessListDB.TABLE_PROCESSES, null, contentValues);
            }

            // clean up old data
            db.delete(
                    ProcessListDB.TABLE_PROCESSES,
                    ProcessListDB.KEY_TIME + " <= ?",
